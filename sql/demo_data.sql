...
BEGIN
  ...
  INSERT INTO leads (id, company_id, user_id, empresa, contacto_nombre, contacto_email, contacto_telefono, status, valor_estimado, notas, created_at, updated_at)
  VALUES
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Grupo Industrial MX',    'Carlos Mendoza',  'cmendoza@gipmx.com',    '55-1234-5678', 'en_negociacion',  850000,  'Interesado en módulo completo. Demo agendada.', NOW()-INTERVAL'5d',  NOW()-INTERVAL'1d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Constructora del Norte', 'Ana López',       'alopez@cdnorte.mx',     '81-9876-5432', 'en_negociacion',  1200000, 'Decisión final esta semana. RFP enviada.',     NOW()-INTERVAL'12d', NOW()-INTERVAL'6h'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Farmacéutica Salud+',    'Roberto Sánchez', 'rsanchez@saludmas.mx',  '55-5555-1234', 'cerrado_ganado',  2300000, 'Contrato firmado. Onboarding en proceso.',     NOW()-INTERVAL'20d', NOW()-INTERVAL'2d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Retail Express SA',      'Diana Torres',    'dtorres@retailex.mx',   '33-4444-8888', 'contactado',      450000,  'PyME con 3 sucursales. Interés en finanzas.',  NOW()-INTERVAL'3d',  NOW()-INTERVAL'3d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Transporte Veloz MX',    'Miguel Herrera',  'mherrera@tvmx.com',     '55-7777-2222', 'contactado',      680000,  'Referido por Carlos M. Demo en 3 días.',       NOW()-INTERVAL'1d',  NOW()-INTERVAL'1d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Tech Innovación MX',     'Sofía Ramírez',   'sramirez@techinno.mx',  '55-3333-9999', 'en_negociacion',  920000,  'Necesitan CRM + Finanzas + IA. Presupuesto OK.',NOW()-INTERVAL'7d', NOW()-INTERVAL'2d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Manufactura Precisa SA', 'Jorge Castro',    'jcastro@mprecisa.mx',   '81-2222-4444', 'cerrado_perdido', 1500000, 'Eligieron competencia por precio. Follow-up Q3.',NOW()-INTERVAL'30d',NOW()-INTERVAL'10d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Alimentos del Pacífico', 'Patricia Vega',   'pvega@alpacmx.mx',      '33-8888-3333', 'en_negociacion',  780000,  'Requieren módulo de compras. Aprobación CFO.',  NOW()-INTERVAL'9d',  NOW()-INTERVAL'4h'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Energía Limpia SA',      'Luis Moreno',     'lmoreno@elimpsa.mx',    '55-6666-1111', 'contactado',      1900000, 'Enterprise. 50+ usuarios. Reunión C-Level.',   NOW()-INTERVAL'2d',  NOW()-INTERVAL'2d'),
    (gen_random_uuid(), 'd0000000-0000-0000-0000-000000000001', v_user_id, 'Servicios Digitales MX', 'Elena Ruiz',      'eruiz@sdmx.com.mx',     '55-1111-7777', 'cerrado_ganado',  560000,  'Plan Business. Activo desde esta semana.',     NOW()-INTERVAL'14d', NOW()-INTERVAL'5d')
